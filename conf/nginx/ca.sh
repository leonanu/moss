...
if [ -e "/usr/local/nginx/conf/moss.pem" ];then
    ...
    umask 77
    openssl req -utf8 -newkey rsa:2048 -keyout /usr/local/nginx/conf/moss.key -nodes -x509 -days 1825 -out /usr/local/nginx/conf/moss.crt -set_serial 0 -subj "/C=CN/ST=Moos/L=Moos/O=Moos Inc/CN=$VHOST_DN"
